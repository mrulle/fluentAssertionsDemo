...
namespace Exercises
{
    public class Ex07_ObjectTests
    {
        ...
        public void BeEquivalentTo_Exclusion()
        {
            ...
            // Assert
            instance.Should().BeEquivalentTo(expectedInstance, o => o.Excluding(p => p.Id));
        ...
        public void ExcludingFields_Including()
        {
            ...
            // Assert order, options => options.ExcludingMissingMembers());
            result.Should().BeEquivalentTo(expected, options => options.Including(ctx => ctx.name));
            //throw new NotImplementedException();
        }
